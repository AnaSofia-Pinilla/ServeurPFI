...
module.exports = class AccountsController extends require("./Controller") {
  ...
  change(user) {
    if (this.requestActionAuthorized()) {
      ...
      if (foundUser) {
        let userForEmailConflictTest = this.usersRepository.findByField(
          "Email",
          user.Email
        );
  ...
  deleteAllUsersBookmarks(userId) {
    ...
    for (let bookmark of bookmarks) {
      if (bookmark.UserId == userId) indexToDelete.push(index);
      index++;
    }
  ...
  deleteAllUsersImages(userId) {
    ...
    for (let image of images) {
      if (image.UserId == userId) indexToDelete.push(index);
      index++;
    }
  ...
  deleteAllUsersNews(userId) {
    ...
    for (let newsItem of news) {
      if (newsItem.UserId == userId) indexToDelete.push(index);
      index++;
    }
  ...
  remove(id) {
    if (this.requestActionAuthorized()) {
      ...
      if (this.usersRepository.remove(id)) this.response.accepted();
