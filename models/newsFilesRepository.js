const fs = require("fs");
module.exports = class NewsFilesRepository {
  ...
  static getImageFileURL(GUID) {
    if (GUID != "") return this.getImageFilesFolder() + GUID + ".png";
    return "";
  }
  ...
  static storeImageData(previousGUID, imageDataBase64) {
    if (imageDataBase64) {
      ...
      (async () => {
        const thumbnailImage = await resizeImg(fs.readFileSync(tempFile), {
          width: newWidth,
          height: newHeight,
        });
